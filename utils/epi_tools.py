...
def initial_population(data, strat_vars):
    if (len(strat_vars)>1) & (strat_vars[-1]=='age_group8'):
        data_age = data.dropna(subset = ['age_group8'], how  = 'any')
        P_age = dict(data_age.groupby(['age_group8'])['w'].sum()/sum(data_age['w'])) 
...
def get_vax_scenario_M1(wave,P_age,data_vax, vax_scenario = None):
    if vax_scenario == 'by_age':
        vax_dist_age = get_vax_dict(data_vax, ['age_group8'], P_age, wave) # P(v|age)
